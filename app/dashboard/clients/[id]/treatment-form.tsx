  ...
  const formContent = (
    <form onSubmit={handleSubmit} className="p-4 space-y-3">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-[1fr_7.5rem_9rem] gap-3 items-start">
          ...
          {/* Created by — read-only, only when editing */}
          {treatment?.created_by_name && (
            <div>
              <label className={labelCls}>{t("karton.col.createdBy")}</label>
              <p className="text-sm text-slate-700 py-1">{treatment.created_by_name}</p>
            </div>
          )}
        </div>
